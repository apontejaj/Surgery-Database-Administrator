...
public class askingDoctor extends JFrame implements ActionListener, ListSelectionListener{
	...
	public askingDoctor(String name, msgsModel msgsModelRef, appsModel appsModelRef){
		
		msgsModelPage = msgsModelRef;
		appsModelPage = appsModelRef;

		String dbQuery = "SELECT * FROM `surgery`.`users` WHERE `Name` = '"+name+"';";
		...
		this.add(first);
			...
			JLabel questionLabel = new JLabel ("Which doctor are you looking for?"); // ASK KYLE
	...
	public void valueChanged(ListSelectionEvent arg0) {
		...
		if (counter == 2){
			...
			if(msgsModelPage != null){
				...
				msgsModelPage.lookingForPatient();
				
			}
			
			else if (appsModelPage != null){
				
				appsModelPage.lookingForPatient();
